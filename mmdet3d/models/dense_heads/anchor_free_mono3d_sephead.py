...
class AnchorFreeMono3DSepHead(BaseMono3DDenseHead):
    ...
    def __init__(
        ...
        self.is_only_sepBN = is_only_sepBN
    ...
    def _init_predictor(self):
        ...
        if self.is_only_sepBN == True:
            for i in range(1):
                for j in range(1, self.num_ins):
                    self.conv_cls_prev[j][i].conv = self.conv_cls_prev[0][i].conv
                ...
                for i in range(len(self.group_reg_dims)):
                    for j in range(1, self.num_ins):
                        if len(self.reg_branch[i]) > 0:
                            self.conv_reg_prevs[j][i][k].conv = self.conv_reg_prevs[0][i][k].conv
        '''
        for i in range(len(self.group_reg_dims)):
            for j in range(1, self.num_ins):
                    self.conv_regs[j][i] = self.conv_regs[0][i]
        '''
        ...
        if self.use_direction_classifier:
            ...
            if self.is_only_sepBN == True:
                for i in range(1):
                    for j in range(1, self.num_ins):
                        self.conv_dir_cls_prev[j][i].conv = self.conv_dir_cls_prev[0][i].conv
        ...
        if self.pred_attrs:
            ...
            if self.is_only_sepBN == True:
                for i in range(1):
                    for j in range(1, self.num_ins):
                        self.conv_attr_prev[j][i].conv = self.conv_attr_prev[0][i].conv
    ...
    def init_weights(self):
        ...
        """
        for modules in [self.cls_convs[0], self.reg_convs[0], self.conv_cls_prev[0]]:
            for m in modules:
                if isinstance(m.conv, nn.Conv2d):
                    normal_init(m.conv, std=0.01)
        for conv_reg_prev in self.conv_reg_prevs[0]:
        ...
        for conv_reg in self.conv_regs[0]:
