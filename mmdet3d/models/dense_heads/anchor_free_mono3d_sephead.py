...
class AnchorFreeMono3DSepHead(BaseMono3DDenseHead):
    ...
    def __init__(
            ...
            init_cfg=None,
            num_ins=None):
        super(AnchorFreeMono3DSepHead, self).__init__(init_cfg=init_cfg)
    ...
    def _init_predictor(self):
        ...
        self.conv_cls_prev = nn.ModuleList()
        self.conv_cls = nn.ModuleList()
        for _ in range(self.num_ins):
            self.conv_cls_prev.append(self._init_branch(
                conv_channels=self.cls_branch,
                conv_strides=(1, ) * len(self.cls_branch)))
            self.conv_cls.append(nn.Conv2d(self.cls_branch[-1], self.cls_out_channels,1))

        self.conv_reg_prevs = nn.ModuleList()
        self.conv_regs = nn.ModuleList()
        for _ in range(self.num_ins):
            conv_reg_prevs = nn.ModuleList()
            conv_regs = nn.ModuleList()
            for i in range(len(self.group_reg_dims)):
                reg_dim = self.group_reg_dims[i]
                reg_branch_channels = self.reg_branch[i]
                out_channel = self.out_channels[i]
                if len(reg_branch_channels) > 0:
                    conv_reg_prevs.append(
                        self._init_branch(
                            conv_channels=reg_branch_channels,
                            conv_strides=(1, ) * len(reg_branch_channels)))
                    conv_regs.append(nn.Conv2d(out_channel, reg_dim, 1))
                else:
                    conv_reg_prevs.append(None)
                    conv_regs.append(
                        nn.Conv2d(self.feat_channels, reg_dim, 1))
            self.conv_reg_prevs.append(conv_reg_prevs)
            self.conv_regs.append(conv_regs)

        if self.use_direction_classifier:
            self.conv_dir_cls_prev = nn.ModuleList()
            self.conv_dir_cls = nn.ModuleList()
            for _ in range(self.num_ins):
                self.conv_dir_cls_prev.append(self._init_branch(
                    conv_channels=self.dir_branch,
                    conv_strides=(1, ) * len(self.dir_branch)))
                self.conv_dir_cls.append(nn.Conv2d(self.dir_branch[-1], 2, 1))

        if self.pred_attrs:
            self.conv_attr_prev = nn.ModuleList()
            self.conv_attr = nn.ModuleList()
            for _ in range(self.num_ins):
                self.conv_attr_prev.append(self._init_branch(
                    conv_channels=self.attr_branch,
                    conv_strides=(1, ) * len(self.attr_branch)))
                self.conv_attr.append(nn.Conv2d(self.attr_branch[-1], self.num_attrs, 1))
    ...
    def init_weights(self):
        ...
        """
        for head_ind in range(self.num_ins):
            for modules in [self.cls_convs[head_ind], self.reg_convs[head_ind], self.conv_cls_prev[head_ind]]:
                for m in modules:
                    if isinstance(m.conv, nn.Conv2d):
                        normal_init(m.conv, std=0.01)
            for conv_reg_prev in self.conv_reg_prevs[head_ind]:
                ...
                for m in conv_reg_prev:
                    if isinstance(m.conv, nn.Conv2d):
                        normal_init(m.conv, std=0.01)
            if self.use_direction_classifier:
                for m in self.conv_dir_cls_prev[head_ind]:
                    if isinstance(m.conv, nn.Conv2d):
                        normal_init(m.conv, std=0.01)
            if self.pred_attrs:
                for m in self.conv_attr_prev[head_ind]:
                    if isinstance(m.conv, nn.Conv2d):
                        normal_init(m.conv, std=0.01)
            bias_cls = bias_init_with_prob(0.01)
            normal_init(self.conv_cls[head_ind], std=0.01, bias=bias_cls)
            for conv_reg in self.conv_regs[head_ind]:
                normal_init(conv_reg, std=0.01)
            if self.use_direction_classifier:
                normal_init(self.conv_dir_cls[head_ind], std=0.01, bias=bias_cls)
            if self.pred_attrs:
                normal_init(self.conv_attr[head_ind], std=0.01, bias=bias_cls)
    ...
    def forward_single(self, x, index):
        ...
        for cls_layer in self.cls_convs[index]:
            cls_feat = cls_layer(cls_feat)
        # clone the cls_feat for reusing the feature map afterwards
        clone_cls_feat = cls_feat.clone()
        for conv_cls_prev_layer in self.conv_cls_prev[index]:
            clone_cls_feat = conv_cls_prev_layer(clone_cls_feat)
        cls_score = self.conv_cls[index](clone_cls_feat)

        for reg_layer in self.reg_convs[index]:
        ...
        for i in range(len(self.group_reg_dims)):
            ...
            if len(self.reg_branch[i]) > 0:
                for conv_reg_prev_layer in self.conv_reg_prevs[index][i]:
                    clone_reg_feat = conv_reg_prev_layer(clone_reg_feat)
            bbox_pred.append(self.conv_regs[index][i](clone_reg_feat))
        bbox_pred = torch.cat(bbox_pred, dim=1)

        dir_cls_pred = None
        if self.use_direction_classifier:
            clone_reg_feat = reg_feat.clone()
            for conv_dir_cls_prev_layer in self.conv_dir_cls_prev[index]:
                clone_reg_feat = conv_dir_cls_prev_layer(clone_reg_feat)
            dir_cls_pred = self.conv_dir_cls[index](clone_reg_feat)

        attr_pred = None
        if self.pred_attrs:
            # clone the cls_feat for reusing the feature map afterwards
            clone_cls_feat = cls_feat.clone()
            for conv_attr_prev_layer in self.conv_attr_prev[index]:
                clone_cls_feat = conv_attr_prev_layer(clone_cls_feat)
            attr_pred = self.conv_attr[index](clone_cls_feat)
