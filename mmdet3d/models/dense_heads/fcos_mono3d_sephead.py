...
class FCOSMono3DSepHead(AnchorFreeMono3DSepHead):
    ...
    def __init__(self,
        ...
        self.num_ins=len(self.regress_ranges)
        super().__init__(
            loss_cls=loss_cls,
            loss_bbox=loss_bbox,
            loss_dir=loss_dir,
    ...
    def _init_layers(self):
        ...
        super()._init_layers()
        self.conv_centerness_prev = nn.ModuleList()
        self.conv_centerness = nn.ModuleList()
        for _ in range(self.num_ins):
            self.conv_centerness_prev.append(self._init_branch(
                conv_channels=self.centerness_branch,
                conv_strides=(1, ) * len(self.centerness_branch)) )
            self.conv_centerness.append(nn.Conv2d(self.centerness_branch[-1], 1, 1))
    ...
    def init_weights(self):
        ...
        for head_ind in range(self.num_ins):
            for m in self.conv_centerness_prev[head_ind]:
                if isinstance(m.conv, nn.Conv2d):
                    normal_init(m.conv, std=0.01)
            normal_init(self.conv_centerness[head_ind], std=0.01)
    ...
    def forward(self, feats):
        ...
        return multi_apply(self.forward_single, feats, self.scales,
                           self.strides, list(range(len(self.strides))) )[:5]

    def forward_single(self, x, scale, stride, index):
        ...
        cls_score, bbox_pred, dir_cls_pred, attr_pred, cls_feat, reg_feat = \
            super().forward_single(x, index)

        if self.centerness_on_reg:
            clone_reg_feat = reg_feat.clone()
            for conv_centerness_prev_layer in self.conv_centerness_prev[index]:
                clone_reg_feat = conv_centerness_prev_layer(clone_reg_feat)
            centerness = self.conv_centerness[index](clone_reg_feat)
        else:
            clone_cls_feat = cls_feat.clone()
            for conv_centerness_prev_layer in self.conv_centerness_prev[index]:
                clone_cls_feat = conv_centerness_prev_layer(clone_cls_feat)
            centerness = self.conv_centerness[index](clone_cls_feat)
